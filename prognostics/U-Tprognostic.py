...
def labels_to_bgr(labels):
    ...
    return bgr
def depth_aware_cra_np(pred: np.ndarray,
                       gt: np.ndarray,
                       n_classes: int = 4,
                       focus: str = "union",   
                       alpha: float = 1.0      
    ...
    pred_i = pred.astype(np.int32)
    gt_i   = gt.astype(np.int32)

    diff = np.abs(pred_i - gt_i)
    score = 1.0 - diff / float(n_classes - 1)  # in [0,1]

    if focus == "all":
        mask = np.ones_like(gt_i, dtype=bool)
    elif focus == "gt":
        mask = gt_i > 0
    elif focus == "union":
        mask = (gt_i > 0) | (pred_i > 0)
    else:
        raise ValueError(f"Unknown focus='{focus}', choose from ['all','gt','union'].")

    if mask.sum() == 0:
        
        return float(np.all(pred_i == gt_i))

    w = 1.0 + alpha * (gt_i[mask] / float(n_classes - 1))
    return float((score[mask] * w).sum() / w.sum())
        ...
        with torch.no_grad():
            for t_idx in range(T - 1):
                ...
                for step_idx, pred_mask in enumerate(preds_seq):
                    ...
                    cra = depth_aware_cra_np(pred_mask, true_mask, n_classes=4, focus="union", alpha=1.0)
                    acc.append(cra)
                    all_preds_acc.append(acc)
